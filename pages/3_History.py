import streamlit as st  # Sicherstellen, dass Streamlit importiert wird
import pandas as pd
...
# Stelle sicher, dass der Benutzer eingeloggt ist, wenn er dies nicht ist, leite weiter
if "user" not in st.session_state:
    # Falls der Benutzer nicht eingeloggt ist, leite ihn zu einer Login-Seite
    LoginManager().go_to_login('Home.py')  # Weiterleitung zur Login-Seite
    st.stop()  # Stoppt die Ausführung, wenn der Benutzer nicht eingeloggt ist
# ====== End Login Block ======

# ------------------------------
# Benutzerspezifisches Verzeichnis erstellen, wenn der Benutzer eingeloggt ist
user = st.session_state.get("user")  # Verwenden von get() um KeyError zu vermeiden
...
if user:
    ...
    st.error("Es gab ein Problem beim Laden des Benutzernamens. Bitte versuche dich erneut einzuloggen.")